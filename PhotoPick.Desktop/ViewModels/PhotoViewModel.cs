...
public class PhotoViewModel : INotifyPropertyChanged
{
    ...
    public string? CameraMake => _model.CameraMake;
    public string? LensModel => _model.LensModel;
    ...
    public string FormattedLens => !string.IsNullOrWhiteSpace(_model.LensModel) ? _model.LensModel : "-";

    public string FormattedShutter
    {
        get
        {
            if (!_model.ExposureTime.HasValue || _model.ExposureTime.Value <= 0) return "-";
            double exp = _model.ExposureTime.Value;
            if (exp >= 1.0) return $"{exp:0.#}s";
            int denom = (int)Math.Round(1.0 / exp);
            return $"1/{denom}s";
        }
    }

    public string FormattedAperture => _model.FNumber.HasValue && _model.FNumber.Value > 0
        ? $"f/{_model.FNumber.Value:0.#}"
        : "-";

    public string FormattedIso => _model.Iso.HasValue && _model.Iso.Value > 0
        ? $"ISO {_model.Iso.Value}"
        : "-";

    public string FormattedFocalLength
    {
        get
        {
            if (!_model.FocalLength.HasValue || _model.FocalLength.Value <= 0) return "-";
            string baseFocal = $"{_model.FocalLength.Value:0.#} mm";
            if (_model.FocalLength35mm.HasValue && _model.FocalLength35mm.Value > 0)
            {
                return $"{baseFocal} ({_model.FocalLength35mm.Value}mm eq)";
            }
            return baseFocal;
        }
    }

    public string Formatted35mmEquiv => _model.FocalLength35mm.HasValue && _model.FocalLength35mm.Value > 0
        ? $"{_model.FocalLength35mm.Value} mm"
        : "-";

    public string FormattedMaxAperture => _model.MaxAperture.HasValue && _model.MaxAperture.Value > 0
        ? $"f/{_model.MaxAperture.Value:0.#}"
        : "-";

    public string FormattedExposureBias => _model.ExposureBias.HasValue
    ...
    public string FormattedExposureLine
    {
        get
        {
            var parts = new System.Collections.Generic.List<string>();
            string shutter = FormattedShutter;
            if (shutter != "-") parts.Add(shutter);
            string ap = FormattedAperture;
            if (ap != "-") parts.Add(ap);
            string iso = FormattedIso;
            if (iso != "-") parts.Add(iso);
            if (_model.ExposureBias.HasValue && Math.Abs(_model.ExposureBias.Value) > 0.05)
            {
                parts.Add($"{(_model.ExposureBias.Value > 0 ? "+" : "")}{_model.ExposureBias.Value:0.#} EV");
            }
            return parts.Count > 0 ? string.Join("  •  ", parts) : "-";
        }
    }

    public string FormattedMegapixels
    {
        get
        {
    ...
    public string FormattedFileInfo => $"{_model.Extension.TrimStart('.').ToUpperInvariant()}  •  {_model.FormattedSize}";
    ...
    public PhotoViewModel(PhotoItem model, CullingSession session)
    {
        ...
        _model.PropertyChanged += (s, e) =>
        {
            ...
            else if (e.PropertyName is nameof(PhotoItem.CameraModel) or nameof(PhotoItem.CameraMake) or
                     nameof(PhotoItem.LensModel) or nameof(PhotoItem.Iso) or
                     nameof(PhotoItem.FNumber) or nameof(PhotoItem.ExposureTime) or
                     nameof(PhotoItem.FocalLength) or nameof(PhotoItem.ExposureBias) or
                     nameof(PhotoItem.FlashFired) or nameof(PhotoItem.DateTaken) or
                     nameof(PhotoItem.Width) or nameof(PhotoItem.Height))
            {
                OnPropertyChanged(nameof(FormattedCamera));
                OnPropertyChanged(nameof(FormattedLens));
                OnPropertyChanged(nameof(FormattedShutter));
                OnPropertyChanged(nameof(FormattedAperture));
                OnPropertyChanged(nameof(FormattedIso));
                OnPropertyChanged(nameof(FormattedFocalLength));
                OnPropertyChanged(nameof(FormattedExposureLine));
                OnPropertyChanged(nameof(FormattedMegapixels));
                OnPropertyChanged(nameof(FormattedDimensions));
                OnPropertyChanged(nameof(FormattedFlash));
                OnPropertyChanged(nameof(FormattedDate));
                OnPropertyChanged(nameof(FormattedFileInfo));
            }
        };
    }
    ...
    public void ApplyMetadata(RawPreviewResult res)
    {
        ...
        OnPropertyChanged(nameof(FormattedDate));
        OnPropertyChanged(nameof(FormattedFileInfo));
        OnPropertyChanged(nameof(Formatted35mmEquiv));
        OnPropertyChanged(nameof(FormattedMaxAperture));
        OnPropertyChanged(nameof(FormattedMeteringMode));
        OnPropertyChanged(nameof(FormattedExposureProgram));
        OnPropertyChanged(nameof(FormattedExposureMode));
        OnPropertyChanged(nameof(FormattedWhiteBalance));
        OnPropertyChanged(nameof(FormattedSoftware));
        OnPropertyChanged(nameof(FormattedSerialNumber));
        OnPropertyChanged(nameof(FormattedExposureBias));
    }
