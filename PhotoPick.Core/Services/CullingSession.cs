    ...
    public async Task<int> LoadDirectoryAsync(string directoryPath, IProgress<(int current, int total)>? progress = null, CancellationToken ct = default)
    {
        ...
        foreach (var file in files)
        {
            ...
            if (cachedData.TryGetValue(file, out var cacheInfo))
            {
                ...
                item.FocalLength35mm = cacheInfo.FocalLength35mm;
                item.MaxAperture = cacheInfo.MaxAperture;
                item.MeteringMode = cacheInfo.MeteringMode;
                item.ExposureProgram = cacheInfo.ExposureProgram;
                item.ExposureMode = cacheInfo.ExposureMode;
                item.WhiteBalance = cacheInfo.WhiteBalance;
                item.Software = cacheInfo.Software;
                item.SerialNumber = cacheInfo.SerialNumber;
            }
