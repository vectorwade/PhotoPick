...
public class CatalogDatabase
{
    ...
    private void InitializeDatabase()
    {
        ...
        string[] migrations = [
            ...
            "ALTER TABLE Photos ADD COLUMN FocalLength35mm INTEGER;",
            "ALTER TABLE Photos ADD COLUMN MaxAperture REAL;",
            "ALTER TABLE Photos ADD COLUMN MeteringMode TEXT;",
            "ALTER TABLE Photos ADD COLUMN ExposureProgram TEXT;",
            "ALTER TABLE Photos ADD COLUMN ExposureMode TEXT;",
            "ALTER TABLE Photos ADD COLUMN WhiteBalance TEXT;",
            "ALTER TABLE Photos ADD COLUMN Software TEXT;",
            "ALTER TABLE Photos ADD COLUMN SerialNumber TEXT;"
        ];
    ...
    public async Task SaveOrUpdatePhotosAsync(IEnumerable<PhotoItem> photos)
    {
        ...
        cmd.CommandText = """
            INSERT INTO Photos (
                ...
                FocalLength35mm, MaxAperture, MeteringMode, ExposureProgram, ExposureMode, WhiteBalance, Software, SerialNumber
            )
            VALUES (
                ...
                $focal35, $maxAp, $metering, $expProg, $expMode, $wb, $soft, $serial
            )
            ON CONFLICT(FilePath) DO UPDATE SET
                ...
                FocalLength35mm = coalesce(excluded.FocalLength35mm, Photos.FocalLength35mm),
                MaxAperture = coalesce(excluded.MaxAperture, Photos.MaxAperture),
                MeteringMode = coalesce(excluded.MeteringMode, Photos.MeteringMode),
                ExposureProgram = coalesce(excluded.ExposureProgram, Photos.ExposureProgram),
                ExposureMode = coalesce(excluded.ExposureMode, Photos.ExposureMode),
                WhiteBalance = coalesce(excluded.WhiteBalance, Photos.WhiteBalance),
                Software = coalesce(excluded.Software, Photos.Software),
                SerialNumber = coalesce(excluded.SerialNumber, Photos.SerialNumber);
        """;
        ...
        var pFocal35 = cmd.Parameters.Add("$focal35", SqliteType.Integer);
        var pMaxAp = cmd.Parameters.Add("$maxAp", SqliteType.Real);
        var pMetering = cmd.Parameters.Add("$metering", SqliteType.Text);
        var pExpProg = cmd.Parameters.Add("$expProg", SqliteType.Text);
        var pExpMode = cmd.Parameters.Add("$expMode", SqliteType.Text);
        var pWb = cmd.Parameters.Add("$wb", SqliteType.Text);
        var pSoft = cmd.Parameters.Add("$soft", SqliteType.Text);
        ...
        foreach (var p in photos)
        {
            ...
            pFocal35.Value = (object?)p.FocalLength35mm ?? DBNull.Value;
            pMaxAp.Value = (object?)p.MaxAperture ?? DBNull.Value;
            pMetering.Value = (object?)p.MeteringMode ?? DBNull.Value;
            pExpProg.Value = (object?)p.ExposureProgram ?? DBNull.Value;
            pExpMode.Value = (object?)p.ExposureMode ?? DBNull.Value;
            pWb.Value = (object?)p.WhiteBalance ?? DBNull.Value;
            pSoft.Value = (object?)p.Software ?? DBNull.Value;
            pSerial.Value = (object?)p.SerialNumber ?? DBNull.Value;
    ...
    public async Task<Dictionary<string, CachedPhotoMetadata>> GetCachedPhotoDataForDirectoryAsync(string directoryPath)
    {
        ...
        while (await reader.ReadAsync())
        {
            ...
            int? focal35 = reader.IsDBNull(17) ? null : reader.GetInt32(17);
            double? maxAp = reader.IsDBNull(18) ? null : reader.GetDouble(18);
            string? metering = reader.IsDBNull(19) ? null : reader.GetString(19);
            string? expProg = reader.IsDBNull(20) ? null : reader.GetString(20);
            string? expMode = reader.IsDBNull(21) ? null : reader.GetString(21);
            string? wb = reader.IsDBNull(22) ? null : reader.GetString(22);
            string? soft = reader.IsDBNull(23) ? null : reader.GetString(23);
            string? serial = reader.IsDBNull(24) ? null : reader.GetString(24);
